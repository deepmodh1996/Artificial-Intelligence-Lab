...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    # added feature should be representative of all the pixel values
    # hence keeping feature as function of number of filled pixels
    # after trying out different possible combinations, submitting the feature which gives heighest accuracy
    numPixel = DIGIT_DATUM_WIDTH*DIGIT_DATUM_HEIGHT
    filledPixel = 0
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if datum.getPixel(x, y) > 0:
                filledPixel = filledPixel + 1

    # features["percentage"] = filledPixel

    features["ratio"] = 1.0*(numPixel-filledPixel)/filledPixel
    # features["ratio2"] = 1.0*numPixel/filledPixel

    # if(filledPixel>(numPixel/6)):
    #     features["filledPixel"] = numPixel/filledPixel
    # else:
    #     features["filledPixel"] = 0
