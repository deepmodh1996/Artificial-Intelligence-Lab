    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
        	...
        	if noisyDistance == None:
	        	...
	        	likelihood = emissionModel[trueDistance]
	        	if likelihood > 0:
	        		posterior[p] = prior[p] * likelihood
        	sumP += posterior[p]
        if noisyDistance == None:
	    	posterior[self.getJailPosition()] = 1.0
        
        if sumP == 0.0 and noisyDistance != None:
        	# print "aaaa"
        	self.initializeUniformly(gameState)
        else:
        	# print posterior
	        ...
	        for i in range(0, self.numParticles):
	        	# posterior.normalize()
	        	self.particles.append(util.sample(posterior))
        # if numZero == len(prior):
...
class JointParticleFilter:
    ...
    def elapseTime(self, gameState):
		...
		"*** YOUR CODE HERE ***"
		for i in range(0, self.numGhosts):
			positionDistibution = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i, self.ghostAgents[i])
			newParticle[i] = util.sample(positionDistibution)
